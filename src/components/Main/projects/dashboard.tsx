...
const Dashboard = () => {
  const projectCode = 'DASHBOARD_ADMIN'
  const projectId = '001'
  const status = 100
  ...
  // Определяем статус на основе distortion
  const getStatusColor = (dist: number) => {
    if (dist < 5) return 'text-terminal-green' // STABLE
    if (dist < 15) return 'text-terminal-warning' // CORRECTION NEEDED
    return 'text-terminal-danger' // INSTABILITY
  }
  ...
  return (
    <div className="w-full flex flex-col space-y-3">
      {/* Project Header with Coordinates */}
      <div className="border-b border-terminal-green/50 pb-3 space-y-2">
        ...
        {/* Status and Coordinates in one row */}
      ...
      <div className="border border-terminal-green/50 bg-black p-3 space-y-2">
        <div className="font-mono text-xs md:text-sm text-terminal-green/80 line-clamp-2">
          Real-time dashboard system for data visualization and management
        </div>
      </div>
      ...
      <div className="flex gap-3 pt-1">
        <Button asChild className="flex-1 terminal-button project-cta-button project-cta-button-filled font-mono text-sm md:text-base uppercase tracking-wider py-3">
          <a href="https://inm1nd.github.io/react_admin/" target="_blank" rel="noopener noreferrer">
            [&gt;] OPEN PROJECT
          </a>
        </Button>
