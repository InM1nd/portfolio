...
const EXPERIENCE = [
  {
    ...
    company: 'Marswell Media',
    points: [
      'Built and maintain marswell.media — bilingual production site on Next.js, TypeScript, Sanity CMS, GSAP, Three.js.',
    ...
    <div className="grid grid-cols-1 gap-5 p-4 md:grid-cols-[210px_1fr] md:p-5">
          ...
          <dl className="space-y-1 font-mono text-[11px]">
            {LANGUAGES.map(([name, level]) => (
              <div key={name} className="flex justify-between gap-2">
                <dt className="text-terminal-green/70">{name}</dt>
                <dd className="text-terminal-green/35">{level}</dd>
              </div>
            ))}
          </dl>
        </Block>
        ...
        <Block label="EDUCATION">
          <ul className="space-y-2 font-mono text-[10px]">
            {EDUCATION.map(([title, place]) => (
              <li key={title}>
                <p className="text-terminal-green/70">{title}</p>
                <p className="text-terminal-green/30">{place}</p>
              </li>
            ))}
          </ul>
        </Block>
      ...
      <div className="flex flex-col gap-5">
        <header>
          ...
          <p className="mt-0.5 font-mono text-[11px] uppercase tracking-[0.2em] text-terminal-green/45">
            Frontend / product engineer · Vienna, AT
          </p>
          <p className="mt-2 max-w-2xl font-mono text-xs leading-relaxed text-terminal-green/70" style={glow}>
            ...
            {EXPERIENCE.map((job) => (
              <li key={job.period} className="border-l border-terminal-green/25 pl-3">
                <div className="flex flex-wrap items-baseline justify-between gap-x-3">
                  <p className="font-mono text-sm uppercase tracking-wide text-terminal-green" style={glow}>
                    {job.role}
                  </p>
                  <p className="font-mono text-[10px] tabular-nums text-terminal-green/35">{job.period}</p>
                </div>
                <p className="font-mono text-[11px] text-[#FFB642]/75">{job.company}</p>
                <ul className="mt-1 space-y-0.5">
                  {job.points.map((point) => (
                    <li key={point} className="font-mono text-[11px] leading-snug text-terminal-green/60">
                      <span className="text-terminal-green/25">— </span>
                      {point}
                    </li>
                  ))}
                </ul>
                <p className="mt-1 font-mono text-[10px] text-terminal-green/30">{job.stack}</p>
              </li>
            ))}
          </ol>
        </Block>
        ...
        <Block label="STACK">
          <div className="grid grid-cols-1 gap-x-6 gap-y-1 sm:grid-cols-2">
            {STACK.map(([group, items]) => (
              <p key={group} className="font-mono text-[11px] text-terminal-green/65">
                <span className="text-terminal-green/30">{group} </span>
                {items}
              </p>
            ))}
          </div>
        ...
        <Block label="HOW I WORK">
          <ul className="grid grid-cols-1 gap-1 font-mono text-[11px] text-terminal-green/60 sm:grid-cols-2">
