...
- Таймлайн (`team.tsx:33-48`) — роли без компаний, без продуктов, без результата. И он расходится
  с резюме: там опыт «с 2021, около четырёх лет» и реальные работодатели (Marswell Media,
...
вопреки резюме. Стало: Marswell Media, Kicks Space, Menudget с реальными задачами; образование
...
8. Удалить шкалы навыков и `CLEARANCE: EXPERT`; вместо этого — стек списком и таймлайн
   с реальными компаниями из резюме (Marswell Media, Kicks Space, Menudget). Починить
...
3. `marswell.media` (текущий работодатель в резюме) не резолвится — ни A, ни NS. `marswell.com`
   резолвится в AWS, но связь не подтверждена. Стоит проверить, что рекрутер увидит.
